...
function Vendors(props: VendorProps): JSX.Element {
    ...
    function getLabelByContinentCode(code) {
        let label = ''
        const [continent, country] = code.split('-')

        countryIsoList.forEach(countryIso => {
            if (continent === 'ALL') {
                label = 'Worldwide'
            }
            if (countryIso.continentCode === continent) {
                label = countryIso.continentName
            }
        });
        return label
    }
    ...
    const availableContinents = vendors.map(v => v.country).reduce((res, country) => {
        const continentCode = country.split('-')[0]
        const [con1, con2] = continentCode.split(',')

        if (!res.includes(con1)) {
           res.push(con1)
        }
        if (!res.includes(con2)) {
            res.push(con2)
        }
    ...
    const filteredVendorLists = vendors.reduce((accum, vendor) => {
        availableContinents.map((continent, jdx) => {
            // if 2 continentcodes are listed, the vendor get put in the section of the continentcode that's listed first
    ...
    var sortedVendors =  filteredVendorLists.reduce(function(sortedVendors, field, index) {
                    ...
                    Object.keys(sortedVendors).map(key => (
                        <div className="continent-section">
                            <div className="continent-bar">
                                <h2 className={'no-bold left'}> { getLabelByContinentCode(key) } </h2>
                            </div>
